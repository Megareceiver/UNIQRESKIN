...
function getTransactions($debtorno, $stat_time='',$end_time='', $reference =0 , $show_also_allocated) {
	if( !$end_time ){
		$end_time = date('Y-m-d');
	}
	...
	AND ".TB_PREF."debtor_trans.due_date <= '$end_time') AS OverDue
	FROM ".TB_PREF."debtor_trans
	WHERE ".TB_PREF."debtor_trans.tran_date <= '$end_time' AND ".TB_PREF."debtor_trans.debtor_no = ".db_escape($debtorno)."
	...
	$where_time = '';
	if( $stat_time ){
		$where_time.=' AND '.TB_PREF.'debtor_trans.tran_date >= '.db_escape($stat_time);
	}
	if( $end_time ){
		$where_time.=' AND '.TB_PREF.'debtor_trans.tran_date <= '.db_escape($end_time);
	}

	if( $reference ){
		$where_time = '';
		$sql.=' AND '.TB_PREF.'debtor_trans.reference ='.db_escape($reference);
	}

	if( $where_time ){
		$sql .= ' AND ('.substr($where_time,4).')';
	}

	// 	if( $stat_time ){
	// 		$sql.=' AND '.TB_PREF.'debtor_trans.tran_date >= '.db_escape($stat_time);
	// 	}
...
function print_statements() {
	global $path_to_root, $systypes_array, $ci;
	...
	$reference = $ci->input->post('PARAM_8');

// 	if( isset($_POST['PARAM_8']) && !$reference ){
// 		display_error(_("You must enter Credit Number."));
// 		return;
// 	}
	// 	bug($reference);die;
	...
	while ($myrow=db_fetch($result)){
		...
		$TransResult = getTransactions($myrow['debtor_no'], $start_date, $end_date, $reference , $show_also_allocated);
