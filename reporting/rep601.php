	...
	if ($prev_balance != 0.0 || $rows != 0)
	{
		...
		if ($rows > 0) {
			...
			while ($myrow=db_fetch($trans)) {
				...
				switch ($myrow["type"]){
				    case ST_BANKPAYMENT:
				        $person_name = payment_person_name($myrow["person_type_id"],$myrow["person_id"], false);
// 				        $person_name = get_customer_name($myrow["person_id"]);
				        break;
			        case ST_CUSTPAYMENT:
			            $person_name = get_customer_name($myrow["person_id"]);
			            break;
				    default:
// 				    case ST_CUSTPAYMENT:
				        if( is_numeric($myrow["person_id"]) ){
				            $person_name = get_customer_name($myrow["person_id"]);
				        } else {
				            $person_name = $myrow["person_id"];
				        }
// 				        $person_name = get_customer_name($myrow["person_id"]);
				        break;
