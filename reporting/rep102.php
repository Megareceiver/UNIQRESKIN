...
// 	$alloc = 'debtor_trans.alloc';
	$alloc = $allocation_model->alloc_sum('debtor_trans.trans_no','debtor_trans.type', $to);
// 	$alloc = "(IFNULL(IF(debtor_trans.type=".ST_SALESINVOICE.",$invoice_alloc_sum,$payment_alloc_sum),0))";
	...
	if ($all)
		...
		$value = "(".TB_PREF."debtor_trans.ov_amount + ".TB_PREF."debtor_trans.ov_gst + ".TB_PREF."debtor_trans.ov_freight + ".TB_PREF."debtor_trans.ov_freight_tax + " .TB_PREF."debtor_trans.ov_discount - ($alloc))";
	...
	WHERE ".TB_PREF."debtor_trans.type <> ".ST_CUSTDELIVERY." AND debtor_trans.type <> ".ST_BANKPAYMENT."
	...
	$sql .= " AND ".TB_PREF."debtor_trans.`trans_no` NOT IN ( SELECT ".TB_PREF.".voided.id FROM ".TB_PREF.".voided AS voided WHERE ".TB_PREF.".voided.type=".TB_PREF."debtor_trans.type )";
...
// bug(ROOT . "/reporting/includes/pdf_report.inc");
