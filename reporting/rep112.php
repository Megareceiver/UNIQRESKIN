...
//----------------------------------------------------------------------------------------------------
function get_receipt($type, $trans_no,$stat_time='',$end_time='',$reference='')
{
...
function print_receipts()
{
	...
	$reference = $_POST['PARAM_7'];

// 	if( isset($_POST['PARAM_7']) && !$reference ){
// 		display_error(_("You must enter Receipts Number."));
// 		return;
// 	}
	...
	for ($i = $from; $i <= $to; $i++) {
		...
		foreach ($types as $j)
		{
			$myrow = get_receipt($j, $i,$start_date,$end_date,$reference);
			if (!$myrow)
				continue;
