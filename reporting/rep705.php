...
function getPeriods($yr, $mo, $account, $dimension, $dimension2,$date_end=NULL)
{
//     $date_end = null;
    if( is_date($date_end) ){
        $date = new DateTime($date_end);
        $date->modify('first day of this month');
        $date13 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date12 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date11 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date10 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date09 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date08 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date07 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date06 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date05 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date04 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date03 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date02 = $date->format('Y-m-d');
        $date->modify('-1 month');
        $date01 = $date->format('Y-m-d');
...
function display_type ($type, $typename, $yr, $mo, $convert, &$dec, &$rep, $dimension, $dimension2, $tags , $date_end=NULL)
{
	...
	while ($account=db_fetch($result))
	{
		...
		$bal = getPeriods($yr, $mo, $account["account_code"], $dimension, $dimension2 , $date_end);
	...
	while ($accounttype=db_fetch($result))
	{
		...
		$totals_arr = display_type($accounttype["id"], $accounttype["name"], $yr, $mo, $convert, $dec, $rep, $dimension, $dimension2, $tags, $date_end);
		for ($i = 1; $i <= 12; $i++)
		{
...
function print_annual_expense_breakdown()
{
	...
	if ($dim == 2)
	{
		$year = input_val('PARAM_0');
		$dimension = input_val('PARAM_1');
		$dimension2 = input_val('PARAM_2');
		$tags = ((input_val('PARAM_3')) ? input_val('PARAM_3') : -1);
	...
	else if ($dim == 1)
	{
		$year = input_val('PARAM_0');
		$dimension = input_val('PARAM_1');
		$tags = ((input_val('PARAM_2')) ? input_val('PARAM_2') : -1);
		$comments = input_val('PARAM_3');
		$orientation = input_val('PARAM_4');
		$destination = input_val('PARAM_4');
	}
	else
	{
		$year = input_val('PARAM_0');
		$tags = ((input_val('PARAM_1')) ? input_val('PARAM_1') : -1);
		$comments = input_val('PARAM_2');
		$orientation = input_val('PARAM_3');
		$destination = input_val('PARAM_4');
	}
	if ($destination)
