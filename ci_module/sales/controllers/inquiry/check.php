...
class SalesInquiryCheck  {
    ...
    function index(){
        ...
        $fields = array(
            'debtor_no' => array('type'=>'CUSTOMER','title'=>_('Customer'),'value'=>0 ),
        ...
        add_js('js/table.js');
        page("Customer Check Transaction");
    ...
    private function trans_tottaly($fields){
        ...
        $detail_count = "(Select COUNT(id) FROM debtor_trans_details AS d WHERE d.debtor_trans_type=tran.type AND d.debtor_trans_no=tran.trans_no)";
        $allocated_count = "(Select COUNT(id) FROM cust_allocations AS allo WHERE allo.trans_type_from=tran.type AND allo.trans_no_from=tran.trans_no)";

        $this->db->select("IF(tran.type=".ST_CUSTPAYMENT.",$allocated_count,$detail_count) AS detail_count",false);
...
//         $this->db->where_in('tran.type',array(ST_CUSTDELIVERY));
