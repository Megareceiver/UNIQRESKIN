...
class GlReportTransactions {
    ...
    function index(){
        ...
        $fromacc =  input_val('PARAM_2');
        $toacc =    input_val('PARAM_3');
...
//         $fromacc =  $toacc = 1300;
        ...
        if ($dim == 2)
        {
            $dimension =    input_val('PARAM_4');
            $dimension2 =   input_val('PARAM_5');
        ...
        if ($dim == 2)
            ...
            while ($account=db_fetch($accounts))
            {
                ...
                if ($rows > 0)
                {
                    ...
                    {
                        if( in_array($myrow["type"], array(ST_SALESINVOICE,ST_CUSTCREDIT)) ){
                            $customer_tran = $ci->db->where(array('trans_no'=>$myrow["type_no"],'type'=>$myrow["type"]))->get('debtor_trans')->row();
                            if( !is_object($customer_tran) ){
                                $update_model->remove_gl($myrow["type"],$myrow["type_no"]);
                                continue;
                            }
                        } elseif( in_array($myrow["type"], array(ST_SUPPINVOICE,ST_SUPPCREDIT)) ){
                            $supplier_tran = $ci->db->where(array('trans_no'=>$myrow["type_no"],'type'=>$myrow["type"]))->get('supp_trans')->row();
                        ...
                        if( in_array($myrow["type"], array(ST_CUSTPAYMENT,ST_SUPPAYMENT)) ){
                        ...
                        if( $myrow["type"]==ST_SALESINVOICE ){ //|| $myrow["type"]==ST_CUSTPAYMENT
                            ...
                            $invoice = $ci->db->select('ref')->where(array('trans_no'=>$myrow["type_no"],'type'=>$myrow["type"]))->get('bank_trans')->row();
