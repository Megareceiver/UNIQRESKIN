...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
   ...
   /**
     ...
     * @return PHPExcel_Worksheet|PHPExcel_Cell    Depending on the last parameter being specified
     */
    public function setCellNumber($pCoordinate = 'A1', $pValue = null, $returnCell = false)
    {
        if( !isset($this->_parent->showNumberZero) ){
            $this->_parent->showNumberZero = true;
        }
        if( $this->_parent->showNumberZero || abs($pValue) > 0 ){
            $cell = $this->getCell($pCoordinate)->setValue($pValue);
            $this->getStyle($pCoordinate)->getNumberFormat()->setFormatCode($this->_parent->numberFormat);
        }
