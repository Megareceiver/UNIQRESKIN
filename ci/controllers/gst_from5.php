...
class Gst_from5 extends ci {
	...
	private function form(){
	    ...
	    page(_('GST Form 5 Report'), false, false, "", $js);
...
// 	        'bug' => array('type'=>'HIDDEN','value'=>1 ),
	    ...
	    if( $submit_button ){
	        echo '<center>';
	        submit_center_first('UPDATE_ITEM','Print GST Form 5', _('PDF Creating'), false);
// 	        submit('ouput_xml', _("Export IAF"), true, _('Submit'));
	    }
	    end_form();
	    end_page();
	}
 ...
 */
	    ...
	    if( $gl_trans && count($gl_trans) > 0 ){ foreach ($gl_trans AS $gl){
	        ...
	        if( !isset($Balance[$gl->account]) ){
	            ...
	            $data['gl_xml'] .= $this->ci->view('gst_check/gst_form5/gl_openning',$linedata_open,true);
	        }
	        ...
	        $data['gl_xml'] .= $this->ci->view('gst_check/gst_form5/gl',$linedata,true);
	    }}
...
// 	    $data['purchase_xml'] = $data['supply_xml'] = null;
	    $xml = $this->ci->view('gst_check/gst_form5/gst-form5-xml',$data,true);
...
// 	        bug($linedata);
	        switch ($type){
	            case 'supply':
	                $xml .= $this->ci->view('gst_check/gst_form5/supply',$linedata,true); break;
	            case 'supplier':
	                $xml .= $this->ci->view('gst_check/gst_form5/purchase',$linedata,true); break;
	            default:break;
	        }
...
//         bug( $this->db->last_query()  );die;
	...
	private function bank_trans($tax_id=0,$tran_type=0){
	    ...
	    $this->db->select('bt.trans_no, bt.type ,inv.currence AS currency_code, inv.tax AS tax_type_id, ABS(inv.amount) AS unit_price, inv.currence_rate AS rate');
	    $this->db->from('bank_trans_detail AS inv');
	    $this->db->where('bt.amount !=',0);
	    $this->db->select('0 AS tax_inclusive, ABS(inv.amount) AS amount',false);
	    if( $tax_id ){
...
// 	    bug( $this->db->last_query() );
	...
	function from5_output(){
	    $data = array(
	        ...
	        'company'=>get_company_prefs(),'created'=>date('Y-m-d'),'date_from'=>$this->date_from,'date_to'=>$this->date_to
	    );
...
// 	    die('end here');
	    ...
	    $this->ci->view('gst_check/gst_form5/form5',$data);
