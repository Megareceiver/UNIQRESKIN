...
class CI_DB_active_record extends CI_DB_driver {
	...
	public function join($table, $cond, $type = '')
	{
		...
		if( is_array($cond) ) {
		    foreach ($cond AS $key=>$key2){
		        $cond_str .="AND $key = $key2 ";
		    }
		    $cond_str = trim(substr($cond_str, 3));
		    unset($cond);
		    $cond = $cond_str;
		}
	...
	/**
	 * Left Join created QuanNH 2016-03-29
	 *
	...
	public function left_join($table, $cond) {
	    ...
	    return $this->join($table, $cond_str,'LEFT');
	}
	 ...
	 * @return	object
	 */
	function insert($table = '', $set = NULL,$returnQuery=false){
		if ( ! is_null($set)){
			$this->set($set);
		}
		...
		foreach ($set AS $field_name=>$val){
			if ( !$this->field_exists($field_name, $table) ){
				$this->query("ALTER TABLE `$table` ADD `$field_name` VARCHAR(255) NOT NULL ");
			}
		}
	 ...
	 * @return	object
	 */
	public function update($table = '', $set = NULL, $where = NULL, $limit = NULL,$returnQuery=false){

		if ($returnQuery && $set != null ){
			$this->_reset_write();
		} else {
	 ...
	 * @return	object
	 */
	public function delete($table = '', $where = '', $limit = NULL, $reset_data = TRUE,$returnQuery=false)
	{
		// Combine any cached components with the current statements
		$this->_merge_cache();
