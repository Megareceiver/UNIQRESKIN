...
function validate(e) {
	if (e.name && (typeof _validate !== 'undefined') && (typeof _validate[e.name] == 'function') )
		return _validate[e.name](e);
	else {
		var n = e.name.indexOf('[');
		if(n!=-1) {
			var key = e.name.substring(n+1, e.name.length-1);
			if (key.length>1 && typeof _validate !== 'undefined' && _validate[e.name.substring(0,n)])
...
function _set_combo_select(e) {
		...
		e.onblur = function() {
			...
			if ((this.selectedIndex != this.getAttribute('_last'))
				||(( jQuery(this).hasClass('combo') || jQuery(this).hasClass('combo3') ) && _update_box(this))
				)
					this.onchange();
		};
		...
		e.onchange = function() {
			...
			if(jQuery(s).hasClass('combo') || jQuery(this).hasClass('combo3')){
				_update_box(s);
			}
	...
	function(e) {
			e.onclick = function() {
				if (validate(e)) {
					...
					if( e.getAttribute('ajax') && e.getAttribute('ajax') != 1 ){
...
//			  _w.moveTo(50, 50);
	...
	'select': function(e) {
		...
		if ( e.className.indexOf("combo") != -1 ){
		    _set_combo_select(e);
		} else {
			e.onkeydown = function(ev) {	// block unintentional page escape with 'history back' key pressed on buttons
				ev = ev||window.event;
		 ...
		 || e.className == 'menu_tab'
 		 || e.className == 'selected')
			e.onclick = function(ev) {
				if (typeof _validate !== 'undefined' && _validate._processing && _validate._modified && !confirm(_validate._processing)) {
					ev.returnValue = false;
					return false;
				}
...
//	    }
	...
	*/
	'ul.ajaxtabs':	function(ul) {
		...
		button = $(this).find('button').click(function() {
...
function setHotKeys() {
	document.onkeydown = function(ev) {
		...
		if ( typeof editors !== 'undefined' && editors[key]) {
			callEditor(key);
