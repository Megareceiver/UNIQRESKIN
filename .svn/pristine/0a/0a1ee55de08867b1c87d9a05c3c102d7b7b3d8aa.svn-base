...
class renderer {
    ...
    function menu_header($title, $no_menu, $is_index,$button_reload=false){
    	global $path_to_root, $help_base_url, $db_connections,$power_by;
    	$logo = '<img src="'.$path_to_root.'/themes/'.user_theme().'/images/logo.png" >';
    	$system_config = $this->model->get_row("name='coy_logo'",'sys_prefs');
    	if( $system_config && isset($system_config['value']) ){
    		$logo = '<img src="'.$path_to_root.'/company/0/images/'.$system_config['value'].'" style="height:64px;" >';
    	}
//     	bug($system_config);
    	...
    	if (!$no_menu) {
                ...
                echo "<script type='text/javascript' src='$path_to_root/js/jquery-ui.js'></script>";
        ...
        function menu_footer($no_menu, $is_index)
        {
            ...
            if ($no_menu == false)
            {
                ...
                if ($no_menu == false)
                       ...
                       echo "<p><a target='_blank' href='$power_url'><font >Copyright &copy; 2014 by $power_by</font></a></p>\n";
