...
function getTaxTransactions($from, $to, $tax_id)
{
    ...
    $sql = "SELECT d.debtor_no, d.name AS cust_name, d.tax_id, dt.type, dt.trans_no,
            ...
            CASE WHEN dt.type=" . ST_CUSTCREDIT . " THEN (ov_gst)*-1
			ELSE (ov_gst) END *dt.rate AS gst_total,
                st.tax_included

		FROM " . TB_PREF . "debtor_trans dt
			LEFT JOIN " . TB_PREF . "debtors_master d ON d.debtor_no=dt.debtor_no
		    LEFT JOIN " . TB_PREF . "sales_types st ON st.id=d.sales_type
...
//             $tax += $taxes['tax'];
//         }
        if ( ($gst = floatval(ABS($trans['gst_total']))) != 0) {
            if ($trans['tax_included'])
                $trans['total'] -= $gst;
