...
if( isset($this->params['delivery_info']) && is_array($delivery_info = $this->params['delivery_info']) && count($delivery_info) >0 ){
    ...
    if( $contact['name'] ){
        $this->NewLine ();
        $delivery_line ++;
        $this->Text ( $ccol, 'Contact Person : '.$contact['name'], $icol );
    }

    if( $contact['email'] ){
        $this->NewLine ();
        $delivery_line ++;
        $this->Text ( $ccol, 'Email : '.$contact['email'], $icol );
    }
    if( $contact['phone'] ){
        $this->NewLine ();
        $delivery_line ++;
        $this->Text ( $ccol, 'Tel : '.$contact['phone'], $icol );
    }
...
//     $delivered_address = "afdsaf lfdsadfsa";
...
$this->addTextWrap(
    ...
    "Please quote Invoice no. when paying. All amounts stated in - ".$this->company['curr_default']
    ,"C");

if( !array_key_exists('bank_account', $this->params) ){
    $this->params['bank_account'] = NULL;
}
$this->addTextWrap(
    $this->leftMargin, 100 - $this->lineHeight*2,
    $width = $this->pageWidth - $this->rightMargin,
    $height= $this->lineHeight +10,
    "Bank: Maybank Berhad, Bank Account: ".$this->params['bank_account']
    ,"C");
...
$this->addTextWrap(
    ...
    $width = $this->pageWidth - $this->rightMargin,
    $height= $this->lineHeight +10,
    "IMPORTANT:Please reference our invoice number on remittance."
    ,"C");
...
$this->addTextWrap(
    ...
    $width = $this->pageWidth - $this->rightMargin,
    $height= $this->lineHeight +10,
    "All cheques to be crossed and drawn to the order of ".strtoupper($this->company['coy_name'])
    ,"C");
$this->addTextWrap(
    $this->leftMargin, 100 - $this->lineHeight*5,
    $width = $this->pageWidth - $this->rightMargin,
    $height= $this->lineHeight +10,
