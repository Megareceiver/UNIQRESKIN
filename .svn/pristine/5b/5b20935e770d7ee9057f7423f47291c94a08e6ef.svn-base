...
//----------------------------------------------------------------------------------------------------
function get_remittance($type, $trans_no,$stat_time='',$end_time='',$reference='')
{
	...
	$where_time = '';
	if( $stat_time ){
		$where_time.=' AND '.TB_PREF.'supp_trans.tran_date >= '.db_escape($stat_time);
	}
	if( $end_time ){
		$where_time.=' AND '.TB_PREF.'supp_trans.tran_date <= '.db_escape($end_time);
	}

	if( $reference ){
		$where_time = '';
		$sql.=' AND '.TB_PREF.'supp_trans.reference ='.db_escape($reference);
	}
	if( $where_time ){
	...
	//    	if( $stat_time ){
	//    		$sql.=' AND '.TB_PREF.'supp_trans.tran_date >= '.db_escape($stat_time);
	//    	}
	//    	if( $end_time ){
	//    		$sql.=' AND '.TB_PREF.'supp_trans.tran_date <= '.db_escape($end_time);
	//    	}
	//    	if( $reference ){
	//    		$sql.=' AND '.TB_PREF.'supp_trans.reference ='.db_escape($reference);
	//    	}
	...
	*/
	...
	for ($i = $from; $i <= $to; $i++)
	{
		...
		foreach ($types as $j) {
			...
			$myrow = get_remittance($j, $i,$start_date,$end_date,$reference);
			if (!$myrow)
				continue;
