	...
	$tags, &$pg, $graphics)
{
	...
	$gl_account_model = module_model_load('accounts','gl');
	$result = get_gl_accounts(null, null, $type);


    $accounts = $gl_account_model->get_gl_accounts(null, null, $type);
//     bug($accounts);
// die("aaa");
    $acc_balance_total_check = 0;
    while (list($index,$account)=each($accounts))
...
// 	    }
	    ...
	    if (!$per_balance && !$acc_balance)
	        continue;
// bug($account);
// bug($acc_balance_total_check);
	    $acc_balance_total_check += abs($acc_balance*$convert);
	}
// die();
...
// 		$rep->AmountCol(4, 5, abs(($acc_balance/$acc_balance_total_check)*100), $pdec);
	...
	if (($code_per_balance + $per_balance_total + $code_acc_balance + $acc_balance_total) != 0 || $printtitle){
		...
		$rep->NewLine();
		//$rep->Font('bold');
// 		if( $typename=='Sales Revenue' ){
// 			$rep->TextCol(0, 2,	_('Gross Profit/(Loss)'));
// 		} else {
			$rep->TextCol(0, 2,	_('Total') . " " . $typename);

// 		}
		//$rep->TextCol(0, 2,	_('Total') . " type name here" . $typename);
// 		$rep->TextCol(2, 3,	"abcdef");
...
function print_profit_and_loss_statement()
{
	...
	while ($class = db_fetch($classresult)) {
		...
		while ($accounttype=db_fetch($typeresult)) {
			...
			if( $accounttype["name"]=='Sales Revenue' || $accounttype['name']=='Other Revenue'){
                $Profit_Loss[0] += ($classtotal[0]*$convert);
                $Profit_Loss[1] += ($classtotal[1]*$convert);
			} else if ($accounttype["name"]=='Cost of Goods Sold'){
...
// 			    $rep->NewLine(2);
			    ...
			    $rep->Font('bold');
			    $rep->TextCol(0, 2,	_('Gross Profit/(Loss)'));
			    $rep->AmountCol(2, 3, $Profit_Loss[0], $dec);
			    $rep->AmountCol(3, 4, $Profit_Loss[1], $dec);
			    $rep->AmountCol(4, 5, Achieve($Profit_Loss[0], $Profit_Loss[1]), $pdec);
			    $rep->Font();
			    $rep->NewLine();
			}
		}
		...
		$rep->NewLine();
		$rep->Font('bold');
// 		if( $class['class_name']=='Sales Revenue' ){
// 			$rep->TextCol(0, 2,	_('Gross Profit/(Loss)'));
// 		} else {
...
// 		}
	...
	$rep->Font('bold');
	$rep->TextCol(0, 2,	_('Net Profit/(Loss)'));
