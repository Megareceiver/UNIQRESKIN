...
class ManufacturingWorkOrderEntry
{
...
//         $this->check_requirement();
    ...
    {
        $item_check = check_db_has_manufacturable_items(
                _("There are no manufacturable items defined in the system."));
        $location_check = check_db_has_locations(
                ("There are no inventory locations defined in the system."));
        if ($item_check || $location_check) {
            end_page();
            exit();
        }
    }
    ...
    private function check_submit ()
    {
        ...
        if (isset($_POST['ADD_ITEM']) && can_process()) {
            ...
            $id = add_work_order($_POST['wo_ref'], $_POST['StockLocation'], 
                    input_num('quantity'), $_POST['stock_id'], $_POST['type'], 
                    $_POST['date_'], $_POST['RequDate'], $_POST['memo_'], 
                    input_num('Costs'), $_POST['cr_acc'], input_num('Labour'), 
                    $_POST['cr_lab_acc']);
        ...
        if (isset($_POST['UPDATE_ITEM']) && can_process()) {
            ...
            update_work_order($selected_id, $_POST['StockLocation'], 
                    input_num('quantity'), $_POST['stock_id'], $_POST['date_'], 
    ...
    private function check_actions_finish ()
    {
        if (isset($_GET['AddedID'])) {
            ...
            display_note(
                    get_trans_view_str($stype, $id, _("View this Work Order")), 
                    0, 1);
            
            if ($_GET['type'] != WO_ADVANCED) {
                include_once ($path_to_root . "/reporting/includes/reporting.inc");
                
                submenu_print(_("&Print This Work Order"), ST_WORKORDER, $id, 
                        'prtopt');
                submenu_print(_("&Email This Work Order"), ST_WORKORDER, $id, 
                        null, 1);
                display_note(
                        get_gl_view_str($stype, $id, 
                                _(
                ...
                $ar = array(
                        ...
                        'PARAM_2' => $stype,
                        'PARAM_3' => '',
                        'PARAM_4' => (isset($def_print_orientation) &&
                                 $def_print_orientation == 1 ? 1 : 0)
                );
                display_note(
                        print_link(
                                _(
                                ...
                                702, $ar), 1);
                hyperlink_params("$path_to_root/admin/attachments.php", 
                        _("Add an Attachment"), "filterType=$stype&trans_no=$id");
            }
...
//         start_table(TABLESTYLE2);
        ...
        if (isset($selected_id)) {
            ...
            if ($myrow["closed"] == 1) {
                echo "<center>";
                display_error(
                        _("This work order is closed and cannot be edited."));
                safe_exit();
            }
            ...
            input_ref("Reference", 'wo_ref',$Refs->get_next(ST_WORKORDER));
            
            //wo_types_list_row(_("Type:"), 'type', null);
            wo_types_input(_("Type"), 'type', null);
        }
        ...
        if (get_post('released')) {
            ...
            stock_manufactured_items_input(_("Item"), 'stock_id', null, false, true);
            if (list_updated('stock_id')){
                $Ajax->activate('quantity');
            }
            locations_bootstrap(_("Destination Location:"), 'StockLocation');    
//             locations_list_row(_("Destination Location:"), 'StockLocation', null);
        ...
        if (! isset($_POST['quantity']))
            ...
            $_POST['quantity'] = qty_format($_POST['quantity'], 
                    $_POST['stock_id'], $dec);
        ...
        if (get_post('type') == WO_ADVANCED) {
            ...
            date_row(_("Date Required By") . ":", 'RequDate', '', null, 
                    $SysPrefs->default_wo_required_by());
        } else {
            qty_input(_("Quantity"), 'quantity', null, null, $dec);
            input_date_bootstrap(_("Date"), "date_");
//             date_row(_("Date") . ":", 'date_', '', true);
            hidden('RequDate', '');
            
            $sql = "SELECT DISTINCT account_code FROM " . TB_PREF .
                     "bank_accounts";
            ...
            gl_all_accounts_list_row(_("Credit Labour Account"), 'cr_lab_acc', 
                    null);
            ...
            gl_all_accounts_list_row(_("Credit Overhead Account"), 'cr_acc', 
                    null);
        }
        ...
        if (isset($selected_id)) {
            ...
            submit_cells('UPDATE_ITEM', _("Update"), '', 
                    _('Save changes to work order'), 'default');
            if (get_post('released')) {
