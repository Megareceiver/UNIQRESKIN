...
// 		$invoice_allocations = $this->db->where(array('type'=>'ST_CUSTPAYMENT','debtor_no'=>$invoice->customer_id,'trans_no_to'=>$invoice_no))->get('cust_allocations')->row();
		$this->db->reset();
		$this->cus_trans->update_customer_trans(ST_SALESINVOICE, $invoice_no, array('ov_amount'=>$ov_amount-$ov_discount,'ov_gst'=>$ov_gst,'ov_discount'=>$ov_discount));
...
//             $cash_sales_invoice_gl = get_company_pref('cash_sales_invoice');
//             if ($invoice->payment_terms['cash_sale'] && $cash_sales_invoice_gl ) {
//                 $amount = $items_total + $items_added_tax + $invoice->freight_cost + $freight_added_tax;
//                 $discount = 0; // $invoice->cash_discount*$amount;

//                 $pmtno = write_customer_payment(0, $invoice->customer_id,
//                     $invoice->Branch, $cash_sales_invoice_gl, $date_,
//                     $Refs->get_next(ST_CUSTPAYMENT), $amount-$discount, $discount,_('Cash invoice').' '.$invoice_no);
...
// 		die(' end write invoice no='.$invoice_no);
	...
	function delivery_daysRule($date_to=null,$date_from=null,$days_left=21,$page=1){
	    if($page > 0){
	        $page --;
	    } else {
	        $page = 0;
	    ...
	    $this->db->where( array('tran.type'=>ST_CUSTDELIVERY) );
	    if( isset($date_to) ){

	        $date_current = strtotime($date_to);
	        ...
	        if( $date > 0 ){
	            $this->db->where('tran.tran_date <',date('Y-m-d',$date));
	            $this->db->order_by('tran.tran_date DESC');
	        }

	        if( isset($date_from) ){
	            $this->db->where('tran.tran_date >=',date('Y-m-d',strtotime($date_from)));
	        }
	    }
	    $this->db->where( 'tran.order_ NOT IN (SELECT order_ FROM debtor_trans WHERE type='.ST_SALESINVOICE.')' );

	    $this->db->join('debtors_master AS deb','deb.debtor_no = tran.debtor_no');
	    $this->db->select('tran.*, deb.name AS customer_name');
	    $this->db->from('debtor_trans AS tran');
// 	    bug( $this->db->last_query() );
// 	    bug($items);die;
// 	    $tempdb = clone $this->db;
// 	    $data = $this->db->limit(page_padding_limit,$page*page_padding_limit)->get()->result();
