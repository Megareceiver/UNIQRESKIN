...
// include_once($path_to_root . "/excel/excel_reader2_patch_applied.php");
...
$control_ci = module_control_load('transaction','gl');
//--------------------------------------------------------------------------------------------------
		...
		if ($result) {
			while ($row = db_fetch($result)) {
				...
				$cart->add_gl_item($row['account'], $row['dimension_id'],
					$row['dimension2_id'], $row['amount'], get_post('gst'),null ,$row['memo_']);
			}
		}
...
function check_item_data()
{
	...
	{
		display_error(_("Department is closed."));
...
function handle_update_item()
{
    if($_POST['UpdateItem'] != "" && check_item_data())
    {
    	...
    	$_SESSION['journal_items']->update_gl_item($_POST['Index'], $_POST['code_id'],
    	    $_POST['dimension_id'], $_POST['dimension2_id'], $amount,  get_post('gst'),null , $_POST['LineMemo']);
    }
	line_start_focus();
}
...
function handle_new_item()
{
	...
	$_SESSION['journal_items']->add_gl_item($_POST['code_id'], $_POST['dimension_id'],$_POST['dimension2_id'], $amount, get_post('gst'),null, $_POST['LineMemo']);
	line_start_focus();
}
