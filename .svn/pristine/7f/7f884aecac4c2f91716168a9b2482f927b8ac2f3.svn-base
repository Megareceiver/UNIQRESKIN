...
class at_Cart {
    ...
    //
    function __construct($type=NULL, $trans_no=0, $prep_child=false) {
        ...
        if( $type !=NULL ){
            ...
            $this->point_model = load_module_model('point',true,'accounting');
            $this->pos = $this->point_model->get_sale_point(user_pos());
        ...
        if ($trans_no[0]) { // read old transaction
            if ($type == ST_SALESORDER || $type == ST_SALESQUOTE) { // sales order || sales quotation
                ...
                if ($this->order_no) { // free hand credit notes have no order_no
                    $sodata = get_sales_order_header($this->order_no, ST_SALESORDER,null,null,0,false);
                    if( !isset($sodata["customer_ref"]) )
                        return false;
                    ...
                    if( $this->trans_type ==  ST_CUSTCREDIT && !empty($this->trans_no)){

                    } else  if (!$prep_child) // this is read for view/reedition
    ...
    function write($policy=0) {
        ...
        switch($this->trans_type) {
            case ST_SALESINVOICE:
                ...
                $ret = $sale_model->write_sales_invoice($this);
                break;
            case ST_CUSTCREDIT:
    ...
    // 	return 0;
    // }
    function add_to_cart_new($tax_type_id, $line_no, $stock_id, $qty, $price, $disc, $qty_done=0, $standard_cost=0, $description=null, $id=0, $src_no=0,
        $src_id=0)
    {
        //TUANVT4
        $line = new line_details($tax_type_id, $stock_id, $qty, $price, $disc,
            $qty_done,  $standard_cost, $description, $id, $src_no, $src_id);
    ...
    //TUANVT4
    function update_cart_item_new($tax_type_id,$line_no, $qty, $price, $disc, $description=""){
        if ($description != "")
            $this->line_items[$line_no]->item_description = $description;
        $this->line_items[$line_no]->tax_type_id = $tax_type_id;
        $this->line_items[$line_no]->quantity = $qty;
        $this->line_items[$line_no]->qty_dispatched = $qty;
        $this->line_items[$line_no]->price = $price;
        $this->line_items[$line_no]->discount_percent = $disc;
    ...
    var $tax_total_dispatch = 0;
    function get_items_total_dispatch() {
        $total = 0;
        $this->tax_total_dispatch = 0;
        foreach ($this->line_items as $ln_itm) {
            if( method_exists($ln_itm, 'line_price') ){
                // 		        $price = $ln_itm->line_price();
                // 		        $total += round(($ln_itm->qty_dispatched * $price * (1 - $ln_itm->discount_percent)), user_price_dec());

                $tax = tax_calculator($ln_itm->tax_type_id,$ln_itm->price*$ln_itm->qty_dispatched*(1 - $ln_itm->discount_percent),$this->tax_included);
                // bug($tax);
                $total += $tax->price;
                $this->tax_total_dispatch += $tax->value;
    ...
    // TUANVT5
    function get_taxes_new($customer_tax_id=null,$shipping_cost=null){
        ...
        foreach ($this->line_items as $ln_itm) {
            $items[] = $ln_itm->stock_id;
            $taxs[] = $ln_itm->tax_type_id;

            $qty = ($this->trans_type == ST_SALESORDER) ? 	$ln_itm->quantity : $ln_itm->qty_dispatched;
            if( method_exists($ln_itm, 'line_price') ){
                $prices[] = round($qty* $ln_itm->line_price()*(1 - $ln_itm->discount_percent),  user_price_dec());
            }

        }

        $taxes = get_tax_for_items_new($taxs,$items, $prices, $shipping_cost, null, $this->tax_included,  null);
...
class line_details
{
    ...
    function line_details($tax_type_id, $stock_id, $qty, $prc, $disc_percent,
