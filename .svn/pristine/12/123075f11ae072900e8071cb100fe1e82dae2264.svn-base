...
class Gst_from3 extends ci {
	function __construct() {
	    global $ci;
	    $this->ci = $ci;

	    $this->tax_model = $this->ci->model('tax',true);
	    $this->cus_trans_model = $ci->module_model( 'sales' ,'customer_trans',true);
	    $this->customer_model = $this->ci->model('cutomer',true);
	    $this->supplier_model = $this->ci->model('supplier',true);
	    $this->supp_trans_model = $ci->module_model( 'purchases' ,'supplier_trans',true);;
	    $this->bank_trans_model = $ci->module_model( 'bank' ,'trans',true);
	    $this->db = $ci->db;
	}

	function index(){

	    $this->date_from = $this->ci->input->post('start_date');
	    if( $this->date_from ){
	        $this->date_from = date('Y-m-d',strtotime($this->date_from) );
	    }
	    $this->date_to =  $this->ci->input->post('end_date');
	    if( $this->date_to ){
	        $this->date_to = date('Y-m-d',strtotime($this->date_to) );
	    }

	    $output = ($this->ci->input->post('ouput_xml')) ? 'xml': 'pdf';
	    if( input_val('ouput_text') ){
	        $output = 'tap';
	    }
// 	    $this->date_from = '01-01-2016';
// 	    $this->date_to = '29-02-2016';
	    if( $this->date_from || $this->date_to ){
	        /*
	         * remove Kastam 150921
	         */
	        /*
	        $sale_model = $this->ci->model('sale',true);
            ...
            */
	    	...
	    	if( ($check78 = abs($this->values['5b']) - abs($this->values['6b']))  > 0){
	    		$this->values['7'] = $check78;
	    	} elseif( ($check78 = abs($this->values['5b']) - abs($this->values['6b'])) < 0  ){
	    		$this->values['8'] = $check78;
	    	}
...
// 	    	bug($this->values);die;
	...
	private function form(){
	    ...
	    page(_('GST Form 3'), false, false, "", $js);
	    ...
	    $form = array(
	        'start_date' => array('type'=>'DATE','title'=>_('Start Date'),'value'=>$date_first->format('d-m-Y') ),
	        'end_date' => array('type'=>'DATE','title'=>_('End Date'),'value'=>$date->format('d-m-Y') ),
	    );
	    $submit_button = true;
	    start_form();
	    ...
	    if( $submit_button ){
	        submit_center_first('UPDATE_ITEM','Print GST Form 3', _('PDF Creating'),true);
	        submit('ouput_xml', _("Export GAF"), true, _('Submit'));
	        submit('ouput_text', _("Export TAP"), true, _('Submit'));
...
// bug($this->values);die('check value');
// 		$beginNum10 = 156.65;
// 		$this->pdf->Text(122.8, 156.65+(13.2* ), money_total_format() );
		for ($i=10; $i<=18 ; $i++){
		...
		$msic_items = $this->ci->api->get_data("msic2/msic_items");
		$msics = array();
...
// 		$this->tax_code = $tax_code['options'];
		...
		$tax_code = $this->ci->api_membership->get_data("tax_code");
...
// 		$tax_items = $this->tax_model->tax_type_items();
		$tax_items = $this->ci->api_membership->get_data('taxdetail');
// bug($tax_items);die;
		$this->tax = array();
...
// 		bug(count($customer_trans_old)); die;
// 		bug($this->values);
...
// 		}
	...
	private function gaf_output(){
	    ...
	    $xml = $this->ci->view('reporting/gst-form3-xml',$data,true);
...
// 	    $format = array('5a|5b|6a|6b|9|10|11|12|13|14|16|17|18|i14|c15|i16|c17|i18|c19|i20|c21|i22|c23|c24');
	    $format = array(
	        1=>'5a',
	        2=>'5b',
	        3=>'6a',
