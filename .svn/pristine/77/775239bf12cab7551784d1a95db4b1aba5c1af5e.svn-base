...
/*
	$('#_page_body table tbody td').hover( function(){
	    //var i = $(this).prevAll('td').length;
		var i = $(this).index() ;
	    $(this).parent().addClass('hover');
	    $('#_page_body table tbody td:nth-child(' + (i + 1) + ')').addClass('hover');
	    if( i > 8 ){
	    	$('#_page_body table tfoot td:nth-child(' + (i -7) + ')').addClass('hover');
	    }

	    $('table.gst-grouping-header td:nth-child(' + (i+ 1) + ')').addClass('hover');

	}, function(){
		$('#_page_body table').find('.hover').removeClass('hover');
	});
...
//	jQuery('.inputdate span.icon').click(function(){
//		$(this).prev('input[type=text]').qdate('show');
//	});
	...
	edit : function(rowTaget,rowControl){
		...
		$('select',rowControl).each(function(){
			...
			if( input.attr('name')=='item_description' ){
			    ...
			    value = rowTaget.find("input[name$='[tax_type_id]']").val();
			}
