...
function print_balance_sheet()
{
	...
	if (!is_date_in_fiscalyears($from) || !is_date_in_fiscalyears($to) || strtotime($to) < strtotime($from) ){
	    display_error(_("This period is not exits in the system!")); return;
	}
...
// 	die('add fiscla year');
	...
	$rep->Font();
	$rep->TextCol(0, 2,	_('Current Period Profit & Loss'));
	if ($lconvert == 1)
	{
		$calc_open *= -1;
		$calc_period *= -1;
